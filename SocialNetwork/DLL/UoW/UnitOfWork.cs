...
namespace SocialNetwork.DLL.UoW
{
    ...
    public class UnitOfWork : IUnitOfWork
    {
        ...
        /// </summary>
        /// <param name="disposing">Если true, метод вызван явно и должны быть освобождены управляемые ресурсы. Если false, метод вызван из финализатора и не должны освобождаться управляемые ресурсы.</param>
        ...
        /// <returns>
        /// Возвращает экземпляр репозитория для указанного типа TEntity. Если пользовательский репозиторий доступен и параметр hasCustomRepository установлен в true, возвращается пользовательский репозиторий; в противном случае создается и возвращается стандартный репозиторий.
        /// </returns>
